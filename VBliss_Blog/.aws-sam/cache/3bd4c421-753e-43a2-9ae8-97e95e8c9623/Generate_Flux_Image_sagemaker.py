...
def is_huggingface_predictor(obj):
    # Check if the object is an instance of sagemaker.predictor.Predictor
    if isinstance(obj, sagemaker.predictor.Predictor):
        # Check for attributes that are specific to a deployed SageMaker endpoint
        return all(hasattr(obj, attr) for attr in ['endpoint_name', 'predict', 'delete_endpoint'])
    return False
    ...
    try:
        ...
        while wait_time < max_wait_time:
            ...
            if status == 'InService':
                ...
                # Check if predictor is defined and is a HuggingFace model deployment object
                if 'predictor' in globals() and is_huggingface_predictor(predictor):
                    try:
                    ...
                    logger.error(f"Endpoint {endpoint_name} is not in service.")
